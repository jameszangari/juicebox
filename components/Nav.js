          ...
          <div className="flex justify-end w-4/12">
            <Link href="/cart" passHref>
              <a className=" relative" aria-label="cart">
                <Bag />
                {cartItems === 0 ? null : (
                  <span className="absolute top-0 right-0 text-xs bg-black text-white font-semibold rounded-full py-1 px-2">
                    {/* <span className="absolute top-0 right-0 text-xs bg-yellow-300 text-gray-900 font-semibold rounded-full py-1 px-2 transform translate-x-10 -translate-y-3"> */}
                    {cartItems}
                  </span>
                )}
              </a>
